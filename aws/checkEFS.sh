...
for dir in "${dir_list[@]}"; do
  ...
  output=$(tail -n +2 $target_dir/$dir/scoutsuite-results/scoutsuite_results_aws-$dir.js| jq '.' |  jq '.services.efs.regions[] | select(.filesystems | length > 0) | .filesystems | keys[]')
  if [[ -n "$output" ]]; then
