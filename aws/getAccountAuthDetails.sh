...
while read -r ACCOUNT_ID; do
    ...
    # Acquire IAM details for Apeman
    aws iam get-account-authorization-details --output json > /opt/apeman/gaad/$ACCOUNT_ID.json

    echo "✅ Got IAM Account Details for account $ACCOUNT_ID"
    echo
    ((counter++))
done < "$ACCOUNT_FILE"
