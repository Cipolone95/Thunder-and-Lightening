...
echo "====Projects with Key Rotation Issues====="

# Loop through and print each directory name
for dir in "${dir_list[@]}"; do
  echo "====Checking Key Rotation in GCP Project $dir====="

   tail -n +2 scoutReports/$dir/scoutsuite-results/scoutsuite_results_$dir.js | jq '.' | jq --arg dir "$dir" '
if (
  (.services.kms.findings["kms-encryption-keys-not-rotated"].flagged_items > 0)
)
then $dir
else empty
end'
